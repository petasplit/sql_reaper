...
DNS_RESOLVERS = [
    # ── Tier-1: Always-reliable global resolvers (highest priority) ──────────
    # Google Public DNS
    "8.8.8.8","8.8.4.4",
    # Cloudflare (no-log)
    "1.1.1.1","1.0.0.1",
    ...
    # ── Tier-2: Reliable secondary resolvers ────────────────────────────────
    # Verisign
    "64.6.64.6","64.6.65.6",
    # Yandex
    ...
    # CenturyLink DNS
    "205.171.3.65","205.171.202.65","205.171.3.66","205.171.202.66",
    # Verizon DNS
    "4.2.2.1","4.2.2.2",
    # Oracle Dyn
    "216.146.35.35","216.146.36.36",
    ...
    # OpenNIC
    "69.195.152.204","23.94.60.240","128.31.0.72",
    # Public-Root
    "199.5.157.131","208.71.35.137","216.87.155.33",
    # Applied Privacy
    "37.252.185.232","94.130.110.185",
    # CZ.NIC
    "193.17.47.1","185.43.135.1",
    # Australian telcos
    "203.50.2.71","203.50.2.72",
    # WIDE Project
    "203.119.4.4",
    # VNPT Vietnam
    ...
    # Chinese public DNS (Alibaba, Baidu, 114DNS, DNSPod)
    "223.5.5.5","223.6.6.6",
    "180.76.76.76",
    ...
    # Taiwanese DNS (Chunghwa, TWNIC)
    "168.95.1.1","168.95.192.1",
    "101.102.103.104",
    # Japanese DNS (IIJ, BIGLOBE, NTT)
    "210.130.0.1","210.130.1.1",
    "220.110.43.107",
    # Singapore (SingNet, StarHub)
    "165.21.83.88","165.21.100.88",
    "202.166.120.100",
    # Indian DNS (BSNL, Airtel, Jio)
    "210.212.2.20","210.212.2.21",
    "125.22.47.125","125.22.47.126",
    # Brazilian DNS (Embratel, Vivo)
    "200.221.11.100","200.221.11.101",
    "200.196.64.1","200.196.64.2",
    # Argentine DNS (Telecom, Fibertel)
    "200.69.128.1","200.69.128.2",
    # Mexican DNS (Telmex, Axtel)
    "200.33.128.1","200.33.129.1",
    # South African DNS (IS, Mweb)
    "196.60.128.13","196.60.128.14",
    "196.15.128.130",
    # Nigerian DNS (MTN, Airtel)
    "197.255.96.1","197.255.96.2",
    # Egyptian DNS (TE Data, Vodafone)
    "213.181.140.133","195.229.25.25",
    # UAE DNS (Du, Etisalat)
    "195.229.241.222","94.200.200.200",
    # European ISP resolvers
    "194.150.168.168","194.150.168.169",  # Nacamar DE
    "213.196.191.96","213.196.191.97",    # Ziggo NL
    "195.121.7.57","195.121.7.58",        # Numericable FR
    "193.251.80.1","193.251.80.2",        # SFR FR
    "217.0.43.144","217.0.44.144",        # T-Online DE
    "145.0.2.10","145.0.2.11",            # XS4ALL NL
    # SWITCH (Switzerland academic)
    "130.59.31.248","130.59.31.249",
    # GARR (Italian academic/research)
    "193.206.141.1","193.206.141.2",
    # NORDUnet (Nordic academic)
    "193.10.0.1","193.10.0.2",
    # CERN DNS
    "128.142.1.1","128.142.14.1",
    # Internet2 / Abilene
    "198.32.240.8","198.32.240.9",
    # Cloudflare for Families (extra entries)
    "1.1.1.2","1.0.0.2","1.1.1.3","1.0.0.3",
    # Philippine DNS (PLDT, Globe)
    "203.177.126.1","210.213.180.50",
    # Indonesian DNS (Telkom, Indosat)
    "202.134.0.155","203.130.196.5",
    # Vietnamese DNS (VNPT, Viettel)
    "203.162.4.190","203.162.4.191",
    # Malaysian DNS (TM, Maxis)
    "211.25.132.1","211.25.132.2",
    # Thai DNS (TOT, AIS, DTAC)
    "203.154.8.3","203.155.8.3",
    # Turkish DNS (Turk Telekom, Turkcell)
    "195.175.39.39","195.175.39.40",
    "85.97.168.2","212.156.74.1",
    # More Cloudflare edge nodes
    ...
    # Optus (Australia)
    "211.29.132.12","203.50.2.71",
    # ── Extra reliable public resolvers ─────────────────────────────────────
    # Google (additional)
    "8.8.8.8","8.8.4.4",
    # Cloudflare (additional entries)
    "1.1.1.1","1.0.0.1",
    # Quad9 (additional entries)
    ...
    "84.200.69.80","84.200.70.40",
    # OpenNIC (additional)
    "172.98.193.62","192.71.249.83","79.143.183.137",
    # Freenom DNS
    "80.80.80.80","80.80.81.81",
    # SafeDNS (additional)
    "195.46.39.39","195.46.39.40",
    # Censurfridns (Denmark, no filtering)
    "91.239.100.100","89.233.43.71",
    # Pure DNS
...
class Result:
    ...
    parameters:     Dict[str, List[str]]= field(default_factory=dict)   # endpoint → [param]
...
def _get_doh_session():
    """Return shared aiohttp.ClientSession for DoH queries. Lazy-initialized."""
    global _DOH_SESSION
    try:
        import aiohttp as _ah
        if _DOH_SESSION is None or _DOH_SESSION.closed:
            _DOH_SESSION = _ah.ClientSession(
                connector=_ah.TCPConnector(limit=50, ssl=False),
    ...
    def _next_tid(self) -> int:
        async def _t():
            async with self._lock:
                ...
                return tid
        # synchronous version for use in sync context
        self._tid = (self._tid + 1) & 0xFFFF
        if self._tid == 0: self._tid = 1
        return self._tid
...
async def _wildcard(domain: str) -> Set[str]:
    """
    Probe 5 random subdomains to build a comprehensive wildcard IP set.
    Returns a SET of wildcard IPs (handles CDN/load-balanced wildcards).
    Returns empty set if no wildcard detected.
    """
    wc_ips: Set[str] = set()
    # Use different lengths to avoid collisions with real subdomains
    randoms = [
        ''.join(random.choices(string.ascii_lowercase, k=18)),
        ''.join(random.choices(string.ascii_lowercase, k=22)),
        ''.join(random.choices(string.ascii_lowercase, k=15)),
        ''.join(random.choices(string.ascii_lowercase, k=26)),
        ''.join(random.choices(string.ascii_lowercase, k=19)),
    ]
    ...
    if len(resolved_ips) >= 2:
        # Only IPs that appear at least twice across our random probes = wildcard IPs
        from collections import Counter
        counts = Counter(resolved_ips)
        wc_ips = {ip for ip, cnt in counts.items() if cnt >= 2}
    return wc_ips


async def batch_resolve(hosts: List[str], wc: Optional[Set[str]] = None) -> Dict[str, str]:
    """
        ...
        while True:
            ...
            try:
                ...
                if results_a:
                    ...
                    if candidate and candidate not in wc_set:
                        resolved_ip = candidate
                # If no A record, try AAAA (IPv6) — some internal hosts are IPv6-only
                if not resolved_ip:
                    results_aaaa = await fr.resolve(h, "AAAA")
                    if results_aaaa:
                        candidate = results_aaaa[0]
                        if candidate and candidate not in wc_set:
                            resolved_ip = candidate
                # If still nothing, try CNAME resolution via DoH (catches CNAME-only
                # responses where the UDP resolver didn't follow the chain)
                if not resolved_ip:
                    results_cname = await fr._doh_resolve(h, "A")
                    if results_cname:
                        candidate = results_cname[0]
                        if candidate and candidate not in wc_set:
                            resolved_ip = candidate
            except Exception:
                pass
            ...
            for match in data.get("matches", []):
                ...
                for h in domains + hostnames:
                    if h and self.d in h.lower():
                        ...
                        # Different domain — could be same org
                        self.r.add_sub(h, "favicon_org") if self.d in h else None
                ip = match.get("ip_str", "")
                if ip:
...
class DNSEnumerator:
    ...
    SRV_PREFIXES = [
        ...
        "_smtp._tcp","_submission._tcp","_submissions._tcp",
    ...
    async def run(self) -> None:
        ...
        log(f"[+] DNS done in {elapsed:.1f}s: {len(self.r.subdomains)} subdomains, "
            f"{sum(len(v) for v in self.r.dns_records.values())} records")
                ...
                if not ip:
                    ...
                    for svc, sigs in TAKEOVER_SIGS.items():
                        for sig in sigs[1:]:  # first element is text sig, rest are domain patterns
                            if any(s in final_cname.lower() for s in [sig]):
                                # Verify by HTTP
                                try:
                                    body = await _tget(
    ...
    async def _check_port(self, ip: str, host: str, port: int) -> Optional[Dict]:
        ...
        async with self._sem:
            ...
            try:
                ...
                # ── HTTP probe ────────────────────────────────────────────────
                if port in (80, 8080, 8000, 8001, 8008, 8081, 8082, 8083,
                ...
                # ── HTTPS probe ───────────────────────────────────────────────
                elif port in (443, 4443, 8443, 8444, 6443, 7443):
                    try:
                        ctx = ssl.create_default_context()
                        ctx.check_hostname = False
                        ctx.verify_mode    = ssl.CERT_NONE
...
class Passive:
    ...
    async def shodaninternet(self) -> None:
        ...
        if len(parts) == 2:
            for last_oct in range(1, 255):
                probe_ip = f"{parts[0]}.{last_oct}"
                await self._jfetch(f"https://internetdb.shodan.io/{probe_ip}", src, cb, timeout=5)
        ...
        def cb(d, s):
            ...
            for e in d.get("entries", []):
                for n in _subs_from_text(str(e), self.d):
                    self.r.add_sub(n, s)
        await self._jfetch(
            f"https://ct.googleapis.com/logs/argon2024/ct/v1/get-entries?start=0&end=10",
            src, cb, timeout=15)
    ...
    async def commoncrawl3(self) -> None:
        ...
        def cb(d, s):
            for n in _subs_from_text(str(d), self.d): self.r.add_sub(n, s)
        try:
            idx_resp = await self._jfetch(
                "https://index.commoncrawl.org/collinfo.json", src, lambda d,s: None, timeout=15)
        except Exception:
            pass
    ...
    async def shodan_ssl(self) -> None:
        src = "shodan_ssl"
        await self._scrape(f"https://internetdb.shodan.io/", src, timeout=15)

    async def fofa_search2(self) -> None:
        src = "fofa_search2"
        await self._scrape(f"https://fofa.info/api/v1/search/all?full=true&fields=host,domain&qbase64=", src, timeout=15)
            ...
            async with self._sem:
                ...
                async with self.s.get(
                    ...
                    if canary in body:
                        ...
                        if reflected:
                            self.r.parameters[f"https://{host}{path}"] = reflected
...
_WAF_BODY_PATS = [
    ...
    re.compile(r'enable\s+javascript.{0,80}(?:security|protection)|just\s+a\s+moment\.{0,5}$', re.I | re.M),
    ...
    re.compile(r'reference\s+#\d+\.\d+\.\d+', re.I),  # Akamai error ref format
    ...
    re.compile(r'naxsi_sig|openresty\s+waf|nginx.*(?:403|access\s+denied)', re.I),
    ...
    re.compile(r'you\s+don.t\s+have\s+permission\s+to\s+access.{0,40}on\s+this\s+server', re.I),
    re.compile(r'access\s+denied.{0,40}(?:nginx|apache|iis|server)', re.I),
    ...
    re.compile(r'<title>[^<]{0,30}(?:403|forbidden|blocked|denied|access\s+denied|security\s+check)[^<]{0,30}</title>', re.I),
...
def _is_waf_response(status: int, headers: dict, body: str) -> bool:
    ...
    if "cf-ray" in hdrs_keys:
        if status in (403, 429, 503):
            return True
        body_snip = (body or "")[:4096].lower()
        if any(p in body_snip for p in ("_cf_chl", "cf-challenge", "jschl_vc",
                                         "cf_clearance", "window._cf_chl_opt")):
            return True

    # ── WAF-identifying headers (presence alone = WAF block likely) ─────────
    waf_only_hdrs = {
        "x-sucuri-id", "x-iinfo", "x-akamai-transformed", "akamai-grn",
        "x-waf-event-info", "x-incap-ses", "x-fw-hash", "x-secintelligence",
        "x-datadome-cid", "x-dd-b", "x-px-cookies", "x-radware", "x-appwall",
        "x-fortinet", "x-fw-server",
    }
    if hdrs_keys & waf_only_hdrs:
        return True

    # ── Status-based quick checks ───────────────────────────────────────────
    if status in (403, 429, 503):
        # Only flag as WAF if there are supporting signals, not bare 403s
        waf_server_sigs = ("cloudflare", "imperva", "incapsula", "sucuri",
                           "akamai", "fastly", "aws", "azure", "ddos-guard",
                           "fortiweb", "radware", "barracuda", "f5")
        server_hdr = hdrs_lower.get("server", "").lower()
        if any(s in server_hdr for s in waf_server_sigs):
            return True
        # Check Azure WAF specific header
        if "x-ms-ref" in hdrs_keys or "x-azure-ref" in hdrs_keys:
            return True

    # ── Body patterns: scan first 8KB ───────────────────────────────────────
    body_snip = (body or "")[:8000]
    for pat in _WAF_BODY_PATS:
        if pat.search(body_snip):
            return True

    # ── Cloudflare JS-challenge even on 200 (Turnstile / 5-second check) ───
    if status == 200 and body:
        ...
        if (("enable javascript" in b and "protection" in b) or
            ...
            "_cf_chl_opt" in b or
            "cf-challenge" in b or
            ("just a moment" in b and "cloudflare" in b) or
            ("checking your browser" in b and "ddos" in b)):
            return True
...
class Prober:
    ...
    def _interesting(self, probe: Dict, baseline: Optional[Dict]) -> bool:
        ...
        if _is_waf_response(st, hdrs, body):
            return False
        # ── WAF challenge body patterns even on 200 ──────────────────────────
        if st == 200 and body:
            b = body[:3000].lower()
            if (("enable javascript" in b and "protection" in b) or
                "please stand by" in b or
                ...
                "checking your browser" in b or
                "datadome" in b or
                "perimeterx" in b or
                ("px.js" in b and ("challenge" in b or "bot" in b))):
                return False
        if st in (200,201,202,204,206): return True
        if st in (401,403,405,407,408): return True
        if st in (301,302,307,308):
            loc = hdrs.get("Location","")
            if loc and not re.search(r'(?:404|error|not.?found)', loc, re.I): return True
        if not baseline: return st < 400
        ...
        if probe.get("hash") != baseline.get("hash") and abs(plen - blen) < 80: return True
        sec_hdrs = {"WWW-Authenticate","X-Auth-Required","X-Frame-Options","Content-Security-Policy"}
        if set(hdrs.keys()) & sec_hdrs - set((baseline or {}).get("hdrs",{}).keys()):
            return True
        return False
    ...
    async def probe_host_paths(self, host: str, paths: List[str]) -> None:
        ...
        if baseline:
            ...
            # If baseline itself is a WAF page (host behind WAF for all paths),
            # skip probing — every path would return WAF page = all false positives
            if baseline.get("waf") and baseline.get("status") in (403, 429, 503):
                log(f"  [WAF] {host} fully WAF-gated (status {baseline['status']}) — skipping path probe")
                return
        ...
        chunk_size = 50   # 50 at a time per host × 3 hosts = 150 max simultaneous
...
class EndpointCrawler:
    ...
    # Case-variation bypass suffixes
    BYPASS_VARIANTS = [
        lambda p: p,                                   # original
        ...
        lambda p: p + '%20',                           # space encoded
        lambda p: '//' + p.lstrip('/'),                # double slash
        lambda p: p + '..;/',                          # Spring bypass
        lambda p: p.replace('/', '/%2f', 1),           # encoded slash
        lambda p: p + ';.json',                        # semicolon bypass
        lambda p: p + '?',                             # empty query
        lambda p: p + '#',                             # fragment bypass
        lambda p: p + '%00',                           # null byte
        lambda p: re.sub(r'/([a-z])', lambda m: '/' + m.group(1).upper(), p, count=1),
    ]
    ...
    CRUD_SUFFIXES = [
        ...
        '/{id}', '/{id}/details', '/{id}/edit', '/{id}/delete',
        '/{id}/update', '/{id}/status', '/{id}/history',
        '/1', '/2', '/3', '/100', '/999',
    ]
    ...
    async def _bfs_crawl(self, host: str) -> None:
        ...
        for scheme in ("https", "http"):
            ...
            async def _worker():
                nonlocal pages
                while not queue.empty() and pages < self.MAX_CRAWL_PAGES:
                    try:
                        url, depth = queue.get_nowait()
                    except Exception:
                        break
                    if url in visited or url in self._crawled:
                        queue.task_done()
                        continue
            ...
            workers = [asyncio.ensure_future(_worker()) for _ in range(20)]
            await asyncio.gather(*workers, return_exceptions=True)
            break  # Try HTTPS only; HTTP is fallback
                ...
                try:
                    async with sem:
                        ...
                        async with self.s.get(
                            ...
                            st = resp.status
                            if st in (200, 201, 202, 204, 301, 302, 307, 308,
                                      401, 403, 405, 406, 407, 408, 500, 501, 502):
                                # Read partial body for verification
                                try:
                                    body = await resp.content.read(2048)
                                    text = body.decode('utf-8', errors='replace')
                                except Exception:
                                    text = ""
                                # For 500s, only record if they look informative
                                if st >= 500 and not any(x in text.lower() for x in
                                ...
                                self.r.live_eps.add(f"{scheme}://{host}{path}")
                                self._found_paths.add(path)
                                # Parse JSON/text response for more paths
                ...
                try:
                    async with sem:
                        ...
                        async with self.s.head(
                            ...
                            if resp.status in (200, 201, 202, 401, 403):
                ...
                try:
                    async with sem:
                        ...
                        async with self.s.head(
                            ...
                            if resp.status in (200, 201, 202, 204, 401, 403, 405):
                ...
                try:
                    async with sem:
                        ...
                        async with self.s.get(
                            ...
                            if resp.status in (200, 201, 202, 204, 302, 401, 403):
    ...
    async def _quick_verify(self, url: str, path: str, source: str) -> bool:
        """Quick HEAD verification of a URL."""
        try:
            async with self._sem:
                to = aiohttp.ClientTimeout(total=4)
                async with self.s.head(
                    ...
                    if resp.status in (200, 201, 202, 204, 301, 302, 401, 403, 405):
                        self.r.live_eps.add(url)
...
class Output:
    ...
    def _write_takeover(self) -> str:
        ...
        with open(p, "w") as f:
            for t in self.r.takeover_candidates:
                f.write(f"{t['subdomain']}\t->\t{t['cname']}\t[{t['service']}]\t"
    ...
    def _write_json(self) -> str:
        ...
        data = {
            ...
            "endpoints": sorted(self.r.live_eps),
    ...
    def _write_html(self) -> str:
        ...
        takeover_rows = ""
        for t in self.r.takeover_candidates:
            takeover_rows += (f"<tr><td>{t['subdomain']}</td><td>{t['cname']}</td>"
                              f"<td>{t['service']}</td><td>{t['confidence']}</td></tr>\n")
...
<title>SQL Reaper v3.0 — {self.domain}</title>
<style>
...
<body>
<h1>🩸 SQL Reaper v3.0 — Recon Report</h1>
...
<div class="section">
<h2>🌐 Live Endpoints (first 500)</h2>
...
async def run(domain: str, cfg: Dict) -> Result:
    ...
    wc_ip = next(iter(wc_ips), None)   # single IP for back-compat with Prober
    if wc_ips:
    ...
    await dns_enum.run()
    log(f"[+] DNS: {sum(len(v) for v in r.dns_records.values())} records")
    ...
    resolved = await batch_resolve(list(all_candidates), wc=wc_ips)
    ...
    HOST_SEM = asyncio.Semaphore(3)    # 3 hosts in parallel — prevents CPU saturation
    ...
    async with aiohttp.ClientSession(
        ...
        async def _scan_host(host: str) -> None:
            async with HOST_SEM:
                ...
                try:
                    await fh._hash_host(f"https://{host}")
                except Exception:
                    pass
                ...
                if not cfg.get("no_params"):
                    host_eps = [ep for ep in r.live_eps if host in ep][:50]
                    if host_eps:
                        try:
                            pd = ParameterDiscovery(session, r, domain, proxy=proxy)
                            await pd.run(host_eps)
                        except Exception:
                            pass
    ...
    log(f"[+] Done: {len(r.live_subs)} subs | {len(r.live_eps)} endpoints | "
        ...
        f"{len(r.secrets)} secrets | {len(r.takeover_candidates)} takeover")
    return r
...
def _args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="reaper_recon",
        description="SQL Reaper v3.0 — Advanced Recon Framework",
    ...
    api.add_argument("--virustotal-key",      metavar="KEY", help="VirusTotal API key")
...
def main() -> None:
    ...
    log(f"[*] SQL Reaper v3.0 — Target: {domain}")
    ...
    print(f"\n{'='*60}")
    print(f"  SQL Reaper v3.0 — Results for {domain}")
    print(f"{'='*60}")
    ...
    print(f"  Secret Leaks     : {len(r.secrets)}")
