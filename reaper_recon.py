...
class Prober:
    ...
    _BYPASS_HEADER_SETS = [
        ...
        # Set 15: Authenticated internal service simulation
        {"Authorization": "Bearer internal-service-token",
         "X-Service-Account": "internal", "X-Forwarded-For": "127.0.0.1"},
    ]
        ...
        try:
            ...
            async with self.s.request(
                ...
                return {
                    ...
                    "body": body[:8192], "scheme": scheme,  # 8 KB so WAF patterns aren't truncated
                }
        except Exception: return None
    ...
    def _interesting(self, probe: Dict, baseline: Optional[Dict]) -> bool:
        ...
        if pt > bt * 3.5 and pt > 0.6:
            return True
        # Same hash → definitely same page (soft-404), skip
        if probe.get("hash") == baseline.get("hash"):
            return False
        # Different hash but very similar length → still interesting
        if probe.get("hash") != baseline.get("hash") and abs(plen - blen) < 120:
            return True
        ...
        try:
            async with sem_pp:
                ...
                async with session.get(
                    ...
                    if st in (401, 403) and len(body.strip()) < 30:
                        return  # Drop — firewall empty deny
