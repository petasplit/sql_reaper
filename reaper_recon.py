...
def _generate_bypass_variants(paths: list) -> set:
    ...
    for path in paths:
        ...
        bypass_paths.add(_fragment_bypass(p))              # /path#/
        ...
        bypass_paths.add(_semicolon_prefix(p))             # ;/path
        ...
        bypass_paths.add(p + '%0a')                        # LF in path
        bypass_paths.add(p + '%0d')                        # CR in path
        bypass_paths.add(p + '%0d%0a/')                   # CRLF bypass
        ...
        # ── Tier 18: Double-encoding bypass (%2561 = double-encoded /) ───────
        double_encoded = p.replace('/', '%2561').replace('%2561', '/', 1)  # keep root slash
...
_WAF_BODY_PATS_EXTRA_2025 = [
    ...
    # Cloudflare Managed Rules 2025
    r'Attention Required|This website is using a security service to protect|one more step',
]
...
def _is_waf_response(status: int, headers: dict, body: str) -> bool:
    ...
    if status in (403, 429, 503):
        ...
        if any(s in server_hdr for s in waf_server_sigs):
            # Unambiguous WAF server headers — return immediately without body check
            _IMMEDIATE_WAF_SERVERS = (
                "cloudflare", "imperva", "incapsula", "sucuri",
            ...
            if any(s in server_hdr for s in _IMMEDIATE_WAF_SERVERS):
                return True
            # Ambiguous WAF/CDN server strings — require additional body or size evidence
            _waf_body_confirm = (
                "access denied" in body_lower or
                "request blocked" in body_lower or
            ...
            if _waf_body_confirm or len(body_snip) < 300:
                return True
    ...
    # Any single weak signal alone may be coincidental; 3+ together is very
    # likely a WAF block page even without a definitive fingerprint.
    _waf_score = 0
    if status in (403, 429, 503):
        _waf_score += 1
    ...
    if "text/html" in hdrs_lower.get("content-type", "") and status in (403, 503):
        _waf_score += 1
    if any(kw in body_lower for kw in ("blocked", "access denied", "forbidden", "protection")):
        _waf_score += 2
    if any(kw in body_lower for kw in ("captcha", "challenge", "verify", "robot", "bot")):
        _waf_score += 2
    if any(kw in body_lower for kw in ("firewall", "security", "threat", "incident")):
        _waf_score += 1
    if hdrs_lower.get("x-cache", "").lower() in ("hit", "miss") and status == 403:
        _waf_score += 1
    # No Set-Cookie and no cache-control often means WAF short-circuit
    if not hdrs_lower.get("set-cookie") and not hdrs_lower.get("cache-control") and status == 403:
        _waf_score += 1
    if _waf_score >= 5:
        return True
...
class Prober:
    ...
    _CHALLENGE_BODY_SIGNALS = frozenset([
        ...
        "kpsdk", "kasada", "kaxsdc", "kcl.js",
        "datadome", "datadome.co", "ddjskey", "ddmc.js",
        "challenge-form", "hcaptcha.com", "arkoselabs.com", "funcaptcha",
        "ak_bmsc", "bm_sz", "bmak.js", "akamai-ghost",
        "shape_utmz", "shapesecurity", "_imp_apg_r_",
        "ntc.js", "netacea.com",
        "rbzid=", "reblaze-proxy",
    ...
    def _interesting(self, probe: Dict, baseline: Optional[Dict]) -> bool:
        ...
        if st in (200, 201, 202, 204, 206):
            # For 200: check it's not a disguised soft-404 or error page
        ...
        if st in (401, 403):
            # 401/403 from a REAL endpoint is interesting, but only if:
            # 1. It differs from the baseline status, OR
            # 2. It has auth-specific response headers, OR
            # 3. Body contains structured error (JSON/XML auth error), OR
            # 4. No baseline to compare against (assume real)
            if not baseline:
                return True
            if st != baseline.get("status", 200):
                return True
            # Check for auth-indicating headers
            auth_hdrs = {"WWW-Authenticate", "X-Auth-Required", "X-Auth-Token",
    ...
    async def probe_host_paths(self, host: str, paths: List[str]) -> None:
        ...
        async def _check(path: str) -> None:
            async with self._sem_h:
                ...
                # ── Multi-method probing: if GET returns 403/405, try HEAD/OPTIONS/POST ──
                # Many WAFs block GET to sensitive endpoints but allow other verbs, and
                # 405 Method Not Allowed from a real endpoint IS interesting (endpoint exists).
                if not interesting and st in (403, 405):
                            ...
                            async with self.s.request(
                                ...
                                if (mm_resp.status in (200, 201, 204, 405, 422) or
                                    ...
                                    if not _is_waf_response(mm_resp.status, dict(mm_resp.headers), mm_body):
                                        ...
                                        self.r.live_eps.add(f"{scheme}://{host}{path} [{method}]")
                                        interesting = True
                                        probe = mm_probe
                                        break
                        except Exception:
...
class EndpointCrawler:
    ...
    BYPASS_VARIANTS = [
        ...
        lambda p: ''.join(c + '​' if c == '/' else c for c in p),       # zero-width space after slash
        lambda p: p.replace('/', '/\t', 1),                                   # tab after first slash
...
async def run(domain: str, cfg: Dict) -> Result:
    ...
    async def _waf_reverify(url: str, session: aiohttp.ClientSession) -> None:
        ...
        2. If WAF blocked, retry with internal-IP bypass headers → if bypass succeeds, keep
        3. If both attempts blocked → discard (false positive)
        """
        # Bypass header sets to try on WAF-blocked re-verify — use all Prober bypass sets
        # so _waf_reverify has the same breadth as live probing
        _REVERIFY_BYPASSES = Prober._BYPASS_HEADER_SETS
        ...
        try:
            async with sem_pp:
                ...
                async with session.get(
                    ...
                    # Only drop empty/tiny 401/403 responses when NOT JSON and no auth header.
                    # A JSON body like {"error":"unauthorized"} IS a real endpoint.
                    if st in (401, 403) and len(body_stripped) < 50:
                        is_json_body = (
                        ...
                        if not is_json_body and not has_auth_hdr:
                            return  # Truly empty/blank WAF deny, not a real endpoint
