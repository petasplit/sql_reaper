...
def _generate_bypass_variants(paths: list) -> set:
    """Generate WAF-bypass path variants for a subset of sensitive paths."""
    _SENSITIVE_TRIGGERS = (
    ...
    bypass_paths = set()
    for path in paths:
        if not any(t in path.lower() for t in _SENSITIVE_TRIGGERS):
            continue
        # Path normalization bypasses
        bypass_paths.add(path.rstrip('/') + '/..')  # trailing parent
        bypass_paths.add(path + ';/')               # Spring semicolon bypass
        bypass_paths.add(path + '%09')              # tab encoding
        bypass_paths.add('/' + path.lstrip('/').replace('/', '//', 1))  # double slash
        bypass_paths.add(path + '?')                # empty query bypass
        bypass_paths.add(path.replace('/', '/%2f', 1))  # encoded first slash
        # Case variants for case-sensitive WAF rules
        if path.lower() != path.upper():
            bypass_paths.add(path.upper())
            # Mixed case (first char of each segment upper)
            bypass_paths.add('/'.join(
                seg.capitalize() if seg else seg
    ...
    async def _worker():
        while True:
            ...
            try:
                ...
                # no redundant explicit _doh_resolve() call needed here.
            except Exception:
                pass
            async with lock:
                if resolved_ip:
...
# Extra 2025-era patterns (referenced in _is_waf_response for additional checks)
_WAF_BODY_PATS_EXTRA_2025 = []  # Already merged into _WAF_BODY_PATS above
...
def _is_waf_response(status: int, headers: dict, body: str) -> bool:
    ...
    if "x-amz-cf-id" in hdrs_keys and status in (403, 503):
        _cf_waf_body = (
            ...
            "<Code>AccessDenied</Code>" in body_snip or  # S3 error — keep!
        ...
        # Keep S3 AccessDenied — that's a real finding (bucket enumerable)
    ...
    if ("x-ms-ref" in hdrs_keys or "x-azure-ref" in hdrs_keys):
        ...
        if status in (403, 429) and _afd_body:
            return True
        if status == 403 and len(body_snip) < 200 and not body_snip.strip().startswith("{"):
            return True   # Tiny non-JSON 403 from AFD = block page
    ...
    if status in (403, 429, 503):
        ...
        if any(s in server_hdr for s in waf_server_sigs):
            # Additionally require a body confirmation for ambiguous servers
            if any(s in ("cloudflare", "imperva", "incapsula", "sucuri",
                          "ddos-guard", "fortiweb", "radware", "aws waf") for s in [server_hdr]):
                return True
            if any(s in server_hdr for s in waf_server_sigs):
                _waf_body_confirm = ("access denied" in body_lower or
                                     "request blocked" in body_lower or
                                     "security" in body_lower and "challenge" in body_lower or
                                     "bot detection" in body_lower or
                                     "captcha" in body_lower)
                if _waf_body_confirm or len(body_snip) < 500:
                    return True
    ...
    for pat in _WAF_BODY_PATS:
        if pat.search(body_snip):
            return True
    for pat in _WAF_BODY_PATS_EXTRA_2025:
        if re.search(pat, body_snip, re.I | re.S):
            return True
...
class Prober:
    ...
    async def _probe(self, host: str, path: str, scheme: str = "https",
        ...
        # ── WAF-block retry with next bypass set ─────────────────────────────
        # If first attempt returned a WAF block page, immediately retry once
        # with the next bypass header set.  This catches WAFs that block the
        # default request but let through "internal" or "health-probe" headers.
        if _retry and result and _is_waf_response(
            ...
            try:
                ...
                async with self.s.request(
                    ...
                    # If retry also returns WAF, return None (don't pollute results)
                    if not _is_waf_response(r2["status"], r2["hdrs"], r2["body"]):
                        return r2
                    return None  # Both attempts WAF-blocked
            except Exception:
    ...
    def _interesting(self, probe: Dict, baseline: Optional[Dict]) -> bool:
        ...
        # ── Extended WAF challenge body check (use full stored 8 KB) ─────────
        if body:
            b = body[:8192].lower()
            if (("enable javascript" in b and "protection" in b) or
                ...
                "kasada" in b or
                "kpsdk" in b or
                ("px.js" in b and ("challenge" in b or "bot" in b)) or
                "human security" in b or
                "humansecurity.com" in b or
                "shape security" in b or
                "threatx" in b or
                "reblaze" in b or
                "approved.by" in b or          # Reblaze
                "akamai-ghost" in b or
                "ak_bmsc" in b or              # Akamai sensor data cookie
                "challenge-form" in b or
                "hcaptcha.com" in b or
                ("recaptcha" in b and "challenge" in b) or
                "cf_chl_rc_m" in b or
                "__cf_chl_f_tk" in b or
                "cf_chl_prog" in b):
                return False
        ...
        if st in (200, 201, 202, 204, 206):
            return True
        if st in (401, 403, 405, 407, 408):
            # 401/403 is interesting only if it differs from baseline or shows auth headers
            if not baseline:
                return True
            if st != baseline.get("status", 200):
                return True
            auth_hdrs = {"WWW-Authenticate", "X-Auth-Required"}
            if set(hdrs.keys()) & auth_hdrs:
                return True
            return False
        if st in (301, 302, 307, 308):
            loc = hdrs.get("Location", "")
            if loc and not re.search(r'(?:404|error|not.?found|blocked|denied)', loc, re.I):
                return True
        if st == 500 and body:
            # Only interesting 500s: those with stack traces / DB errors
            b500 = body[:4096].lower()
            if any(x in b500 for x in (
                'stack', 'trace', 'exception', 'sqlexception', 'pdoexception',
                'activerecord', 'sequelize', 'django.db', 'traceback',
                'at java.', 'caused by:', 'file "/var/', 'line ', 'syntax error'
            )):
                return True
            return False
        ...
        # Only mark as interesting when lengths differ by more than ±10%/200 bytes,
        ...
        if size_ratio < 0.08 and abs(plen - blen) < 200:
            ...
            has_data_structure = (
                ('"id"' in body_lower_check or '"data"' in body_lower_check or
                 ...
                 '"count"' in body_lower_check or '"total"' in body_lower_check) and
                probe.get("ct", "").startswith("application/json")
            )
        ...
        if baseline:
            ...
            if baseline.get("waf") and baseline.get("status") in (403, 429, 503):
                ...
                for bypass_set in self._BYPASS_HEADER_SETS:
                    rand = f"/{''.join(__import__('random').choices(__import__('string').ascii_lowercase, k=22))}"
                    try:
                        ...
                        async with self.s.request(
                            ...
                            if not _is_waf_response(resp_b.status, hdrs_b, body_b):
                                bypass_baseline = {
                                    ...
                                    "hash": __import__('hashlib').md5(body_b.encode()).hexdigest(),
                ...
                if bypass_baseline is None:
                    log(f"  [WAF] {host}: all {len(self._BYPASS_HEADER_SETS)} bypass attempts failed — skipping")
                    return
...
class EndpointCrawler:
    ...
    async def _quick_verify(self, url: str, path: str, source: str) -> bool:
        """GET verification of a URL with WAF false-positive filtering."""
        try:
            async with self._sem:
                to = aiohttp.ClientTimeout(total=5)
                async with self.s.get(
        ...
        try:
            async with sem_pp:
                ...
                async with session.get(
                    ...
                    if _is_waf_response(st, dict(resp.headers), body[:8192]):
                        return  # Drop — WAF block page
                    # Only drop empty/tiny 401/403 responses when NOT JSON.
                    # A JSON body like {"error":"unauthorized"} (25 chars) IS a real
                    # endpoint returning a structured auth error — keep it.
                    if st in (401, 403) and len(body_stripped) < 30:
                        is_json_body = (
                        ...
                        if not is_json_body and not has_auth_hdr:
                            return  # Drop — truly empty/blank WAF deny, not a real endpoint
                    verified_eps.add(url)
        except Exception:
            # Network error — keep the endpoint (could be intermittent)
            verified_eps.add(url)
