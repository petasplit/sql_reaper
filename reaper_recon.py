...
def _is_waf_response(status: int, headers: dict, body: str) -> bool:
    ...
    if "cf-ray" in hdrs_keys:
        if status in (403, 429, 503):
            return True
        if any(p in body_lower for p in ("_cf_chl", "cf-challenge", "jschl_vc",
    ...
    if status == 200 and body:
        if ("datadome" in body_lower or
            "dd.js" in body_lower and "captcha" in body_lower):
            return True
...
class Prober:
    ...
    async def _probe(self, host: str, path: str, scheme: str = "https") -> Optional[Dict]:
        ...
        bypass_set = self._BYPASS_HEADER_SETS[self._bypass_idx % len(self._BYPASS_HEADER_SETS)]
        self._bypass_idx += 1
