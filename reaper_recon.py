...
class EndpointCrawler:
    ...
    BYPASS_VARIANTS = [
        ...
        lambda p: p.replace('/', '//', 1).replace('//', '//', 1),             # double all slashes
...
async def run(domain: str, cfg: Dict) -> Result:
    ...
    async def _waf_reverify(url: str, session: aiohttp.ClientSession) -> None:
        ...
        """
        # Bypass header sets to try on WAF-blocked re-verify
        _REVERIFY_BYPASSES = [
            {"X-Forwarded-For": "127.0.0.1", "X-Real-IP": "127.0.0.1"},
            {"X-Forwarded-For": "10.0.0.1", "X-Real-IP": "10.0.0.1", "True-Client-IP": "10.0.0.1"},
        ...
        try:
            async with sem_pp:
                ...
                async with session.get(
                    ...
                    set_cookie = hdrs_dict.get("Set-Cookie", "")
                    if set_cookie:
                        _waf_cookie_sigs = (
                            "__cfduid", "cf_clearance", "cf_chl_2",
                            "_datadome", "datadome",
                            "_pxhd", "px_fid", "_px3",
