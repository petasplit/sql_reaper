...
class FastUDPResolver:
    ...
    - 2000+ concurrent in-flight queries
    - LRU cache for repeated lookups
    - DoH fallback for blocked UDP
    - Retry with different resolver on timeout
    """
    MAX_INFLIGHT = 8_000
    TIMEOUT      = 2.5    # generous timeout for international resolvers
    RETRY        = 2     # two retries on different resolvers
...
async def batch_resolve(hosts: List[str], wc: Optional[str] = None) -> Dict[str, str]:
    ...
    • Wildcard detection: filter ANY IP matching the wildcard probe
    • No blocking os.getaddrinfo() — stays fully async
    • NWORKERS calibrated to avoid OS socket exhaustion (≤3000 concurrent UDP sockets)
    • Progress every 5 000 hosts
    """
    ...
    total   = len(host_list)
    out: Dict[str, str] = {}
    wc_set  = {wc} if wc else set()
    fr      = _get_fast_resolver()
    counter = [0]

    # Cap workers: > 3000 concurrent UDP queries causes OS-level socket exhaustion
    # and paradoxically slows things down; 2000 is a safe sweet spot.
    ...
    report_every = max(100, total // 40)  # ~40 progress reports total
    ...
    async def _worker():
        while True:
            try:
                h = q.get_nowait()
            except Exception:  # asyncio.QueueEmpty / queue.Empty
                return
            try:
                results = await fr.resolve(h, "A")
                if results:
                    ip = results[0]
                    # Filter wildcards
                    if ip and ip not in wc_set:
                        out[h] = ip
            except Exception:
                pass
            counter[0] += 1
            c = counter[0]
            if c % report_every == 0 or c == total:
                elapsed = time.monotonic() - t_start[0]
                rate = c / elapsed if elapsed > 0.1 else 0
                eta = (total - c) / rate if rate > 0 else 0
                pct = c * 100 // total
                log(f"  [DNS] {c:,}/{total:,} ({pct}%) | live={len(out):,} | "
                    f"{rate:.0f}/s | ETA {eta:.0f}s")

    log(f"  [DNS] Resolving {total:,} hosts — {NWORKERS:,} workers | {len(fr._resolvers)} resolvers …")
    t0 = time.monotonic()
...
class Mutator:
    ...
    def endpoint_mutations(self) -> Set[str]:
        ...
        ext_variants = [".json",".xml",".yaml",".csv",".txt",".html",
                        ";.json","?format=json","?output=json","?accept=json",
                        ".php",".asp",".aspx",".jsp",".do"]
        for path in list(self.r.endpoints)[:400]:
...
_WAF_BODY_PATS = [
    ...
    re.compile(r'enable\s+javascript.{0,80}security|just\s+a\s+moment', re.I),
    ...
    re.compile(r'request\s+blocked.{0,60}security\s+policy', re.I),
    re.compile(r'you\s+don.t\s+have\s+permission\s+to\s+access.{0,40}on\s+this\s+server', re.I),
    re.compile(r'access\s+denied.{0,40}nginx|nginx.{0,40}access\s+denied', re.I),
    re.compile(r'an?\s+error\s+occurred.{0,60}reference\s+number\s*#\d+', re.I),
    re.compile(r'your\s+ip\s+(?:address\s+)?(?:has\s+been\s+)?blocked', re.I),
    re.compile(r'<title>[^<]{0,30}(?:403|forbidden|blocked|denied|access\s+denied)[^<]{0,30}</title>', re.I),
    re.compile(r'security\s+check.{0,60}browser', re.I),
    re.compile(r'verifying\s+you\s+are\s+human', re.I),
    re.compile(r'please\s+stand\s+by.{0,80}ddos\s+protection', re.I),
]
...
def _is_waf_response(status: int, headers: dict, body: str) -> bool:
    ...
    hdrs_lower = {k.lower() for k in headers}
    # Cloudflare WAF: cf-ray + 403/429/503 or JS challenge body
    if "cf-ray" in hdrs_lower:
        if status in (403, 429, 503):
            return True
        body_snip = body[:4096].lower()
        if "_cf_chl" in body_snip or "cf-challenge" in body_snip:
            return True
    # Header-keyed WAF signatures (presence alone = WAF)
    waf_hdrs = {"x-sucuri-id","x-iinfo","x-akamai-transformed","akamai-grn",
                "x-waf-event-info","x-incap-ses","x-fw-hash","x-secintelligence"}
    if hdrs_lower & waf_hdrs:
        return True
    # Body patterns
    body_snip = body[:5000] if body else ""
...
class Prober:
    ...
    def _interesting(self, probe: Dict, baseline: Optional[Dict]) -> bool:
        ...
        if st == 200 and body:
            b = body[:2000].lower()
            if ("enable javascript" in b and "protection" in b) or                "please stand by" in b or "verifying you are human" in b or                "_cf_chl_opt" in b:
                return False
...
async def run(domain: str, cfg: Dict) -> Result:
    ...
    wc_ip = await _wildcard(domain)
    if wc_ip:
        log(f"[!] Wildcard DNS: *.{domain} → {wc_ip}")
    ...
    resolved = await batch_resolve(list(all_candidates), wc=wc_ip)
